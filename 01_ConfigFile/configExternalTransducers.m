...
config.Lbcb1.Base(1,:) = [-496.085,  3.349,  -6.998]'/25.4;           % Base coordinate
config.Lbcb1.Plat(1,:) = [-228.064, -2.602,   0.674]'/25.4;           % Plat coordinate
                                                                
config.Lbcb1.Base(2,:) = [-193.208, -423.583,-4.428]'/25.4;           % Base coordinate    
config.Lbcb1.Plat(2,:) = [-191.633, -0.103,  -6.959]'/25.4;           % Plat coordinate
                                                                
config.Lbcb1.Base(3,:) = [ 187.711, -449.115,-5.257]'/25.4;           % Base coordinate    
config.Lbcb1.Plat(3,:) = [ 189.679, -6.127,  -0.217]'/25.4;           % Plat coordinate
...
config.Lbcb1.McTransform = eye(4);
config.Lbcb2.McTransform = eye(4);
...
config.Params.TOL = [0.0001 0.0001 0.00005 0.00005]';

% Perturbation for jacobian estimation '
config.Params.Dx  = 0.001;
config.Params.Dy  = 0.001;
config.Params.Drz = 0.001;
config.Params.Drx = 0.001;
